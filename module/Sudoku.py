...
class Board:
    ...
    def __init__(
        self, data=[[0 for i in range(9)] for i in range(9)], difficulty="medium"
    ):
        self.size = len(data)
    ...
    def __repr__(self):
        ...
        for i in range(9):
            string += "| {} {} {} | {} {} {} | {} {} {} |\n".format(
                *[str(j.value) for j in self.__data[i]]
            ).replace("0", " ")
            if (i + 1) % 3 == 0:
                string += "+-------+-------+-------+\n"
        return string
...
def __enter():
    ...
    for i in range(9):
        char = input(
            "Enter the {}th row (all-together, eg: 123456789): ".format(str(i + 1))
        )
    ...
    while not (sudoku.validate() and sudoku.is_complete()):
        ...
        if cell.weight() != 8:
            ...
            for i in set(
                [
                    j.value
                    for j in sudoku.row(cell) + sudoku.column(cell) + sudoku.block(cell)
                ]
            ):
...
def batch_solve(
    ...
    *args
    ...
    result = pool.starmap_async(
        algo, tuple((i, visualise) for i in list_of_sudoku)
    ).get()
    return result
