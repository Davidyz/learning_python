...
def is_music(name):
    return (
        name.split(".")[-1]
        in ("mp3", "aac", "m4a", "flac", "ape", "wav", "dsd", "dsf", "dff")
...
class Music:
    ...
    def __init__(self, path, strict_mod=False):
        ...
        if is_music(path):
            ...
            raise InputError(
                "\nInvalid path: {}.\nIt is a directory or is not a recognised music file.".format(
                    path
                )
            )
    ...
    def format(
        self, target=None, form=None, bitrate=320, overwrite=True, bt_protocal="aac"
    ):
        if target == None:
        ...
        if os.path.isdir(target):
            target_file = os.path.sep.join([target, self.__path[-1]]).replace(
                self.form, form
            )
        ...
        codec = {"flac": "", "aac": " -c:a libfdk_aac ", "mp3": " -c:a libmp3lame "}

        command = 'ffmpeg -i "{}" -q 0 -map_metadata 0 "{}" -y -loglevel quiet'.format(
