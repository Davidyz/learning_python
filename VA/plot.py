...
x = [
    i * 0.05
    for i in range(
        math.floor((min(blanks) - 3) / 0.05), math.ceil((max(blanks) + 3) / 0.05)
    )
]
...
plt.plot(
    difficulty, DFS_weight, linestyle="", marker="v", label="DFS Weighted", color="g"
)
...
plt.plot(
    difficulty, BFS_weight, linestyle="", marker="H", label="BFS Weighted", color="y"
)
