...
class Ball(turtle.Turtle):
    ...
    def distance(self):
        global width, height
        return math.sqrt(
            (self.xcor() + width / 2) ** 2 + (self.ycor() + height / 2) ** 2
        )
...
def check_ball_collision(ball1, ball2, e=1 / 2):
    ...
    if distance <= R:
        ...
        ball1.velocity = [
            (2 * ball2.mass * u2x + u1x * (ball1.mass - ball2.mass))
            / (ball1.mass + ball2.mass),
            (2 * ball2.mass * u2y + u1y * (ball1.mass - ball2.mass))
            / (ball1.mass + ball2.mass),
        ]
        ball2.velocity = [
            (2 * ball1.mass * u1x + u2x * (ball2.mass - ball1.mass))
            / (ball1.mass + ball2.mass),
            (2 * ball1.mass * u1y + u2y * (ball2.mass - ball1.mass))
            / (ball1.mass + ball2.mass),
        ]
...
while True:
    ...
    for i in range(0, len(balls) - 1):
        if (
            balls[i + 1].distance() - balls[i].distance() < 50
        ):  # balls[i + 1].radius + balls[i].radius:
