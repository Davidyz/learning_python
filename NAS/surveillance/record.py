...
command = (
    """ffmpeg -i {camera} -c:v copy -r 30 -t 3600 -map 0 {file_name}.mkv""".format(
        camera=IPcam_url, file_name=time.ctime()[4:].replace(" ", "_") + ".mkv"
    )
)
...
while True:
    ...
    old_clip = [
        ...
        if time.time() - os.path.getmtime(i) >= days_saved * 24 * 60 * 60
    ]
    for i in old_clip:
